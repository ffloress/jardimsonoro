...
// Globe
const globeTexture = new THREE.TextureLoader().load("mundo.jpg");
...
const markers = new THREE.InstancedMesh(markerGeometry, markerMaterial, markerData.length);
...
markerData.forEach((data, i) => {
  ...
  const pos = new THREE.Vector3().setFromSphericalCoords(5.1, phi, theta);

  dummy.position.copy(pos);
  dummy.lookAt(pos.clone().setLength(6));
  dummy.updateMatrix();
  markers.setMatrixAt(i, dummy.matrix);
  markerRefs.push({ pos, infoDivId: data.infoDivId, clicked: false });
  phase.push(Math.random());
});
markerGeometry.setAttribute("phase", new THREE.InstancedBufferAttribute(new Float32Array(phase), 1));
scene.add(markers);
...
window.addEventListener("pointerdown", (event) => {
  ...
  const hits = raycaster.intersectObject(markers).filter(m =>
    (m.uv.subScalar(0.5).length() * 2) < 0.25
  );
  if (hits.length > 0) {
    const iid = hits[0].instanceId;
    const ref = markerRefs[iid];
    const div = document.getElementById(ref.infoDivId);
    if (div) {
      labelDiv.innerHTML = div.innerHTML;
      label.position.copy(ref.pos);
      labelDiv.classList.remove("hidden");
    }
    if (!ref.clicked) {
      const greenDotMesh = new THREE.Mesh(greenDot, greenMaterial);
      greenDotMesh.position.copy(ref.pos);
      clickedMarkersGroup.add(greenDotMesh);
      markers.setMatrixAt(iid, new THREE.Matrix4().makeScale(0, 0, 0));
...
// Esferas verdes simples para marcadores clicados
const clickedMarkersGroup = new THREE.Group();
scene.add(clickedMarkersGroup);
const greenMaterial = new THREE.MeshBasicMaterial({ color: 0x32CD32 });
const greenDot = new THREE.SphereGeometry(0.05, 8, 8);
